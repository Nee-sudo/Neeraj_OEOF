...
export async function createElectionCollections(db: any): Promise<void> {
  ...
  const collectionExists = async (name: string): Promise<boolean> => {
    if (typeof db.listCollections !== 'function') {
      // We are likely on the mock database fallback or custom wrapper that doesn't support listCollections.
      // Let's safe-guard by just returning false and attempting collection initialization.
      return false;
    }
    try {
